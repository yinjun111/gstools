...
gseagen
version: $version\n
Usage:  gstools gsea-gen -e ./data/gene.results.merged.tpm.txt -o resultsfolder -s ./data/configV1.txt -n Group -t Human.B38.Ensembl88 -d h.all.v7.4\n
...
Parameters:
    ...
    --runmode|-r      Where to run the scripts, local, cluster or none [none]
                                  ...
                                  local, run the scripts using parallel in local workstation	
                                  cluster, run the scripts in cluster
...
elsif ($tx eq "Rat.Rn6.Ensembl88") {
	$chipfile="/data/jyin/Databases/GSEA/msigdb_v7.4_chip_files_to_download_locally/Rat_ENSEMBL_Gene_ID_Human_Orthologs_MSigDB.v7.4.chip";
}
else {
